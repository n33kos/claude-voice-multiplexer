...
if [ ! -f "$CONFIG_FILE" ] || [ "$FORCE" = true ]; then
    ...
    sed \
        ...
        -e "s|__DETECTED_DEVICE__|${DETECTED_DEVICE}|g" \
        -e "s|__LIVEKIT_API_KEY__|${LIVEKIT_API_KEY}|g" \
        -e "s|__LIVEKIT_API_SECRET__|${LIVEKIT_API_SECRET}|g" \
        -e "s|__AUTH_SECRET__|${AUTH_SECRET_VAL}|g" \
        "$SCRIPT_DIR/env.template" > "$CONFIG_FILE"
    log "Config written to $CONFIG_FILE (from env.template)"
else
