#!/usr/bin/env bash
#
# claude-voice-multiplexer:start
#
# DEPRECATED: In v2.0+, services are managed by the vmuxd daemon (launchd).
# This script is kept as a backwards-compatibility wrapper.
#
# For new installs, the daemon auto-starts via launchd on login.
# Use `vmux status` to check service status.
# Use `vmux shutdown` to stop everything.
#
# This script will try the daemon first, then fall back to the old direct-start
# approach for environments without launchd (e.g. CI, Linux).
#

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
DATA_DIR="$HOME/.claude/voice-multiplexer"

# --- Try vmuxd first ---
VMUX_CLI="$DATA_DIR/daemon/vmux"
if [ -f "$VMUX_CLI" ]; then
    # Check if daemon is already running
    if "$VMUX_CLI" status &>/dev/null 2>&1; then
        echo "vmuxd daemon is running. Use 'vmux status' for details."
        echo "Services are managed automatically — no need to run start.sh manually."
        exit 0
    fi
    echo "Daemon not responding. Check: launchctl start com.vmux.daemon"
    exit 1
fi

# --- Legacy fallback (pre-daemon install) ---
echo "Warning: vmuxd not installed. Falling back to legacy start mode."
echo "Run ./scripts/install.sh to set up the daemon."
echo ""

# Load configuration
if [ -f "$DATA_DIR/voice-multiplexer.env" ]; then
    set -a
    source "$DATA_DIR/voice-multiplexer.env"
    set +a
fi

PID_FILE="$DATA_DIR/.vmux.pid"

if [ ! -d "$DATA_DIR" ]; then
    echo "ERROR: Voice Multiplexer not installed."
    echo "  Run: ./scripts/install.sh"
    exit 1
fi

WHISPER_PORT="${VMUX_WHISPER_PORT:-8100}"
WHISPER_MODEL="${VMUX_WHISPER_MODEL:-base}"
WHISPER_THREADS="${VMUX_WHISPER_THREADS:-auto}"
KOKORO_PORT="${VMUX_KOKORO_PORT:-8101}"
KOKORO_DEVICE="${VMUX_KOKORO_DEVICE:-auto}"
RELAY_PORT="${RELAY_PORT:-3100}"
LIVEKIT_PORT="${LIVEKIT_PORT:-7880}"
LIVEKIT_API_KEY="${LIVEKIT_API_KEY:-devkey}"
LIVEKIT_API_SECRET="${LIVEKIT_API_SECRET:-secret}"
WEB_PORT="${WEB_PORT:-5173}"
DEV_MODE="${DEV_MODE:-false}"

if [ "$1" = "--dev" ]; then
    DEV_MODE=true
fi

is_vmux_running() {
    if [ -f "$PID_FILE" ]; then
        local pid
        pid=$(cat "$PID_FILE" 2>/dev/null)
        if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
            return 0
        fi
        rm -f "$PID_FILE"
    fi
    if pgrep -f "scripts/start.sh" | grep -v "$$" > /dev/null 2>&1; then
        return 0
    fi
    return 1
}

if is_vmux_running; then
    echo "Claude Voice Multiplexer is already running."
    if [ -f "$PID_FILE" ]; then
        echo "  PID: $(cat "$PID_FILE")"
    fi
    echo "  Use ./scripts/stop.sh to stop it first."
    exit 1
fi

echo $$ > "$PID_FILE"
PIDS=()

cleanup() {
    echo ""
    echo "Shutting down..."
    for pid in "${PIDS[@]}"; do
        kill "$pid" 2>/dev/null || true
    done
    for port in "$WHISPER_PORT" "$KOKORO_PORT" "$LIVEKIT_PORT" "$RELAY_PORT" "$WEB_PORT"; do
        local pids_on_port
        pids_on_port=$(lsof -ti:"$port" 2>/dev/null || true)
        if [ -n "$pids_on_port" ]; then
            echo "$pids_on_port" | xargs kill 2>/dev/null || true
        fi
    done
    sleep 1
    for pid in "${PIDS[@]}"; do
        kill -9 "$pid" 2>/dev/null || true
    done
    rm -f "$PID_FILE"
    echo "All services stopped."
}

trap cleanup EXIT INT TERM

LOG_DIR="$DATA_DIR/logs"
mkdir -p "$LOG_DIR"

rotate_log() {
    local log_file="$1"
    local max_bytes=$((5 * 1024 * 1024))
    if [ -f "$log_file" ] && [ "$(stat -f%z "$log_file" 2>/dev/null || stat --format=%s "$log_file" 2>/dev/null || echo 0)" -ge "$max_bytes" ]; then
        mv "$log_file" "${log_file}.old"
    fi
}

rotate_log "$LOG_DIR/whisper.log"
rotate_log "$LOG_DIR/kokoro.log"
rotate_log "$LOG_DIR/relay.log"
rotate_log "$LOG_DIR/start.log"

log() { echo "$1"; echo "$1" >> "$LOG_DIR/start.log"; }

log "=== $(date '+%Y-%m-%d %H:%M:%S') ==="
log "Starting Claude Voice Multiplexer (legacy mode)..."

export WHISPER_URL="${WHISPER_URL:-http://127.0.0.1:${WHISPER_PORT}/v1}"
export KOKORO_URL="${KOKORO_URL:-http://127.0.0.1:${KOKORO_PORT}/v1}"

wait_for_service() {
    local name="$1" url="$2" timeout="${3:-30}" elapsed=0
    while ! curl -s "$url" > /dev/null 2>&1; do
        sleep 1; elapsed=$((elapsed + 1))
        if [ "$elapsed" -ge "$timeout" ]; then
            log "  WARNING: $name did not respond within ${timeout}s"; return 1
        fi
    done
}

WHISPER_BINARY="$DATA_DIR/whisper/whisper.cpp/build/bin/whisper-server"
WHISPER_MODEL_PATH="$DATA_DIR/whisper/models/ggml-${WHISPER_MODEL}.bin"

if ! curl -s "http://127.0.0.1:${WHISPER_PORT}/" > /dev/null 2>&1; then
    if [ "$WHISPER_THREADS" = "auto" ]; then
        WHISPER_THREADS=$(sysctl -n hw.logicalcpu 2>/dev/null || nproc 2>/dev/null || echo 4)
    fi
    log "  Whisper: starting on :${WHISPER_PORT}..."
    "$WHISPER_BINARY" \
        --host 127.0.0.1 --port "$WHISPER_PORT" \
        --model "$WHISPER_MODEL_PATH" \
        --inference-path /v1/audio/transcriptions \
        --threads "$WHISPER_THREADS" \
        >> "$LOG_DIR/whisper.log" 2>&1 &
    PIDS+=($!)
    wait_for_service "Whisper" "http://127.0.0.1:${WHISPER_PORT}/" 15
    log "  Whisper: running"
fi

KOKORO_REPO="$DATA_DIR/kokoro/kokoro-fastapi"
if ! curl -s "http://127.0.0.1:${KOKORO_PORT}/health" > /dev/null 2>&1; then
    if [ "$KOKORO_DEVICE" = "auto" ]; then
        [ "$(uname)" = "Darwin" ] && KOKORO_DEVICE="mps" || KOKORO_DEVICE="cpu"
    fi
    log "  Kokoro: starting on :${KOKORO_PORT}..."
    (
        cd "$KOKORO_REPO"
        env -i HOME="$HOME" \
            PATH="$KOKORO_REPO/.venv/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
            USE_GPU=true USE_ONNX=false \
            PYTHONPATH="$KOKORO_REPO:$KOKORO_REPO/api" \
            MODEL_DIR=src/models VOICES_DIR=src/voices/v1_0 \
            DEVICE_TYPE="$KOKORO_DEVICE" PYTORCH_ENABLE_MPS_FALLBACK=1 \
            "$KOKORO_REPO/.venv/bin/uvicorn" api.src.main:app \
            --host 127.0.0.1 --port "$KOKORO_PORT" \
            >> "$LOG_DIR/kokoro.log" 2>&1
    ) &
    PIDS+=($!)
    wait_for_service "Kokoro" "http://127.0.0.1:${KOKORO_PORT}/health" 60
    log "  Kokoro: running"
fi

if ! curl -s "http://127.0.0.1:${LIVEKIT_PORT}" > /dev/null 2>&1; then
    log "  LiveKit: starting on :${LIVEKIT_PORT}..."
    livekit-server --bind 0.0.0.0 --keys "${LIVEKIT_API_KEY}: ${LIVEKIT_API_SECRET}" > /dev/null 2>&1 &
    PIDS+=($!)
    sleep 2
fi

mkdir -p "$PROJECT_DIR/web/dist"
log "  Relay server: starting on :${RELAY_PORT}..."
cd "$PROJECT_DIR/relay-server"
uv run \
    --python 3.12 \
    --with "fastapi>=0.110" --with "uvicorn>=0.27" \
    --with "websockets>=12.0" --with "httpx>=0.27" \
    --with "python-dotenv>=1.0" --with "livekit-api>=0.7" \
    --with "livekit>=1.0" --with "numpy>=1.24" \
    --with "scipy>=1.10" --with "webrtcvad>=2.0.10" \
    --with "setuptools" --with "fastmcp>=2.0" \
    --with "PyJWT>=2.8" --with "setproctitle>=1.3" \
    server.py >> "$LOG_DIR/relay.log" 2>&1 &
PIDS+=($!)

if [ "$DEV_MODE" = true ]; then
    log "  Web dev server: starting on :${WEB_PORT}..."
    cd "$PROJECT_DIR/web"
    npm install --silent > /dev/null
    npx vite --port "$WEB_PORT" &
    PIDS+=($!)
fi

LOCAL_IP=$(ipconfig getifaddr en0 2>/dev/null || echo "your-local-ip")
log ""
log "Open http://localhost:${RELAY_PORT} in your browser"
log "Open http://${LOCAL_IP}:${RELAY_PORT} from your phone"
log ""
log "Press Ctrl+C to stop all services."
log ""

wait || true
